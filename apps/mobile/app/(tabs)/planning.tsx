...
const BLOCK_LABELS: Record<BlockType, string> = {
  task: 'Görev', routine: 'Rutin', break: 'Mola',
  focus: 'Odak', meal: 'Yemek', workout: 'Antrenman',
}
...
export default function PlanningScreen() {
  ...
  const { timeBlocks, fetchTimeBlocks, addTimeBlock, deleteTimeBlock } = usePlanningStore()
  ...
  const load = useCallback(async (uid: string, date: string) => {
    await fetchTimeBlocks(uid, date)
  }, [fetchTimeBlocks])
      ...
      >
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
            {weekDays.map((day, i) => {
              ...
              return (
                ...
                >
                  <Text style={{ fontSize: fontSize.xs, color: colors.textSubtle, fontWeight: fontWeight.medium }}>
                    {DAY_LABELS[i]}
                  </Text>
                  <View style={{
                    width: 34, height: 34, borderRadius: 17, alignItems: 'center', justifyContent: 'center',
                    backgroundColor: isSelected ? palette.accent : isToday ? `${palette.accent}18` : 'transparent',
                    borderWidth: isToday && !isSelected ? 1 : 0,
                    borderColor: palette.accent,
                  }}>
                    <Text style={{ fontSize: fontSize.sm, fontWeight: fontWeight.semibold, color: isSelected ? '#fff' : isToday ? palette.accent : colors.textSecondary }}>
                      {day.getDate()}
                    </Text>
                  </View>
                  {dayBlocks.length > 0 && (
                    <View style={{ width: 4, height: 4, borderRadius: 2, backgroundColor: isSelected ? palette.accent : colors.textSubtle }} />
                  )}
                </TouchableOpacity>
              )
            })}
        ...
        {dayBlocks.length === 0 ? (
          ...
          <View style={{ gap: spacing[3] }}>
            {dayBlocks.map((block) => (
              <BlockRow key={block.id} block={block} onDelete={() => deleteTimeBlock(block.id)} />
            ))}
          </View>
        )}
      </ScrollView>
          ...
          <View>
            ...
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: spacing[2] }}>
              {BLOCK_TYPES.map((type) => (
                <TouchableOpacity
                  key={type}
                  onPress={() => setDraft((d) => ({ ...d, block_type: type }))}
                  style={{
                    paddingHorizontal: spacing[3], paddingVertical: 8, borderRadius: radius.md,
                    backgroundColor: draft.block_type === type ? BLOCK_COLORS[type] : colors.glassInner,
                    borderWidth: 1, borderColor: draft.block_type === type ? BLOCK_COLORS[type] : colors.border,
                  }}
                >
                  <Text style={{ fontSize: fontSize.sm, fontWeight: fontWeight.medium, color: draft.block_type === type ? '#fff' : colors.textMuted }}>
                    {BLOCK_LABELS[type]}
                  </Text>
                </TouchableOpacity>
              ))}
  ...
  return (
    <GlassCard padding={spacing[4]} noShadow>
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: spacing[3] }}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={{ fontSize: fontSize.base, fontWeight: fontWeight.medium, color: colors.textPrimary }} numberOfLines={1}>
            {block.label}
          </Text>
          ...
          <View style={{ marginTop: 4, alignSelf: 'flex-start', paddingHorizontal: 8, paddingVertical: 2, borderRadius: 6, backgroundColor: `${color}18` }}>
            <Text style={{ fontSize: fontSize.xs, color, fontWeight: fontWeight.medium }}>
